...
`TestSuite` is versioned and contains only named checks from the core allowlist. The first suite is `adaos-observe-smoke` version `1.0.0`.
...
A product invariant failure or timeout makes the campaign `failed`. An unavailable runner, missing key, disabled node, missing capability, or SSH transport failure is `inconclusive`; it does not mark the tested build defective. All assignments must avoid product failures and quorum must pass.
...
3. Enter the exact commit or build identity in **Prepare observe-only campaign**.
