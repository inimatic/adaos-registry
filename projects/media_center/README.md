...
playback surface now owns exactly one attached session: replacement playback
supersedes prior history, activation repairs legacy duplicates, and stale queue
observations cannot restore a previously selected item. Remote surfaces add
revisioned presentation
...
fullscreen. The agent defaults to portable PyAV/system-ffprobe technical
...
Release `0.6.97` was the first release with resumable stopped sessions. It is
superseded because it could expose more than one recently stopped session for
the same endpoint and accepted an endpoint observation from an older queue
revision during remote selection.
...
paths before handoff. Terminal sessions stay in the endpoint inbox until their
latest command is observed, which makes remote Stop durable across event and
poll timing. Media Server remains an independently
...
Release `0.6.92` was not promoted: its package was assembled before the project
manifest commit, so its immutable source receipt did not identify the exact
manifest state. `0.6.93` is the first deployable release of this component set.
The release adds an authoritative playback
...
instances without a current membership lease or the desired topology generation.
The coordinator returns a deterministic FTS hit immediately without trying to
fill the requested page from fuzzy discovery or source agents. Fuzzy discovery
is an empty-FTS fallback with a 100-row scoring window by default; operators may
raise `MEDIA_CENTER_DISCOVERY_SCORE_CANDIDATES` for recall-sensitive libraries.
Source-agent search remains an empty-index recovery path. On the 68,429-item
`.30` catalog this removed a 26.8-second, 600-candidate page-fill pass from the
exact `Аэроград` query, so an unavailable node cannot delay a valid local hit.
...
Release `0.6.74` is superseded by `0.6.75`. It contains the same bootstrap
implementation, but its immutable receipt was built before the skill release
command normalized the final published version to `0.8.67`.
