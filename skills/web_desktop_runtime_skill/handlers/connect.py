...
_log = logging.getLogger("skills.web_desktop_runtime.connect")
...
async def _finish_prepare(mode: str, webspace_id: str, request_id: str, context: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        _log.warning("desktop prepare_connection failed mode=%s webspace=%s", mode, webspace_id, exc_info=True)
...
async def prepare_connection(
    ...
    **_: Any,
    ...
    return await _prepare_from_payload(payload)


async def _prepare_from_payload(payload: Dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "ok": bool(current and current.get("status") == "ready"),
        "cached": False,
        "current": current or {},
    }
