...
- Every changed source receives a technical descriptor. The default `auto` policy uses a system `ffprobe` when available and otherwise the bundled PyAV wheel; `MEDIA_LIBRARY_AGENT_PROBE_MODE=basic` is an explicit low-cost opt-out. Existing partial descriptors are backfilled once by technical-probe revision, and `MEDIA_LIBRARY_AGENT_PROBE_TIMEOUT_SECONDS` is clamped to 1-60 seconds.
