...
The TLP execution package and primary dataset belong to
``tlp_experiment_skill``. The generic research manager consumes only the
typed runner-provider contract exposed by this skill.
"""
...
def _write_event(stream, *, name: str, value: float, split: str, epoch: int, direction: str, dataset_digest: str, sequence: int) -> None:
    payload = {
        ...
        "aggregation": "mean",
        "observed_at": _now(),
        "producer": {"component": "tlp_experiment_skill.runner", "sequence": sequence},
